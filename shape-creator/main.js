...
hands.onResults(results => {
  // If camera is off, ignore results
  ...
  // Single hand pinch: drag + delete
  for (const landmarks of results.multiHandLandmarks) {
    const indexTip = landmarks[8];
    const position = get3DCoords(indexTip.x, indexTip.y);

    if (isPinch(landmarks)) {
      if (!selectedShape) selectedShape = findNearestShape(position);
      ...
      if (selectedShape) {
        ...
        if (inBin) recycleBin?.classList.add('active');
        else recycleBin?.classList.remove('active');
      }
    } else {
      if (selectedShape && isInRecycleBinZone(selectedShape.position)) {
        scene.remove(selectedShape);
...
const initCamera = async () => {
  ...
  camController = new Camera(video, {
  ...
  camController.start();
  setStatus('Status: camera ON');
};

const stopCamera = () => {
  setStatus('Status: camera OFF');
...
const bindUI = () => {
  ...
  startBtn?.addEventListener('click', async () => {
    try {
      ...
      console.error(e);
      setStatus('Status: camera error (permission?)');
      alert('Camera error. Check browser permission for camera.');
    }
  });
