                ...
                for(var i = event.resultIndex; i < event.results.length; ++i){
                    if (event.results[i].isFinal) {
                        ...
                        if (_this.onTranscript) {
                            _this.onTranscript(_this.finalTranscript, ''); // Send final, clear interim
                        }
                        _this.finalTranscript = ''; // Reset for the next full utterance
                    } else {
                        _this.interimTranscript += event.results[i][0].transcript;
                    }
                    _this.onTranscript(null, _this.interimTranscript); // Send only interim
                }
            };
